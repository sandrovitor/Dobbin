...
- **vendas**: Todas as vendas feitas na plataforma.
...
CREATE TABLE `vendas` (
 ...
 `cliente_id` mediumint(8) unsigned NOT NULL COMMENT 'ID do cliente que efetuou a compra.',
 `roteiro_id` smallint(5) unsigned NOT NULL COMMENT 'ID do roteiro',
 `items` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Items (poltronas, tarifas e descontos) desta venda.',
 `adultos` tinyint(3) unsigned NOT NULL COMMENT 'Quantidade de adultos. Máximo de 100.',
 `criancas` tinyint(3) unsigned NOT NULL COMMENT 'Quantidade de crianças. Máximo de 100.',
 `clientes_total` tinyint(3) unsigned NOT NULL COMMENT 'Poltronas comercializadas nesta venda. Máximo 100.',
 `lista_clientes` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'JSON. Lista de clientes nesta venda.',
 `desconto_total` mediumint(9) NOT NULL COMMENT 'Desconto total (soma dos descontos). Máximo de 9.999.999 centavos.',
 `valor_total` mediumint(9) unsigned NOT NULL COMMENT 'Valor total da venda (com os descontos). Máximo 9.999.999 centavos.',
 `valor_devolvido` mediumint(8) unsigned DEFAULT NULL COMMENT 'Valor devolvido (caso venda seja estornada). Máximo 9.999.999 centavos.',
 `status` enum('Reserva','Aguardando','Paga','Cancelada','Devolvida') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Situação da venda.',
 `obs` varchar(300) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Observação sobre a venda.',
 `forma_pagamento` enum('','Crédito','Débito','Boleto','Digital','Transferência','Dinheiro','Outro') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT 'Forma de pagamento escolhida para o cliente.',
 `vencimento` date DEFAULT NULL COMMENT 'Vencimento do pagamento (BOLETO)',
 `usuario_id` tinyint(4) NOT NULL COMMENT 'ID do usuário que fez a venda.',
 `data_reserva` datetime NOT NULL COMMENT 'Data do pedido da reserva.',
 `data_venda` datetime DEFAULT NULL COMMENT 'Data e hora da venda no sistema.',
 ...
 PRIMARY KEY (`id`),
 UNIQUE KEY `id` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Venda dos roteiros.'
```
