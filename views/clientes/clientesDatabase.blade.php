        ...
        <div class="card">
            ...
            <div class="card-body database" style="overflow-x:auto;">
                <div class="mb-3">
                    ...
                    <button type="button" class="btn btn-info btn-sm" onclick="loadDatabaseClientes()"><i class="fas fa-sync"></i></button>
                </div>
...
<script>
    function loadDatabaseClientes()
    {
        let total = 0;
        //console.log(total);

        $.post(PREFIX_POST+'clientes/database', {ini: 0, qtd: total}, function(res){
            //console.log(res);

            if(res.success == true) {
                dbLocal.clientesDB = res.clientes;
                dbLocal.clientesDBHora = new Date();
                escreveTabela();
            } else {
    ...
    $(document).ready(function(){
        if(dbLocal.clientesDB == "") {
            loadDatabaseClientes();
        } else {
            // Verifica se já passou 10 min ou mais desde a última atualização.
            let agora = new Date();
            if(agora - dbLocal.clientesDBHora >= 10 *60*1000) {
                loadDatabaseClientes();
            } else {
                escreveTabela();
            }
