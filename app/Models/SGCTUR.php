...
DADOS;
    ...
    public function getClienteAniversario(string $data)
    {
        ...
        try {
            ...
            // HOJE
            $abc = $this->pdo->query("SELECT id, nome, cidade, estado, nascimento, email FROM `clientes` WHERE nascimento LIKE '%-".$hoje->format('m-d')."'");
            if($abc->rowCount() > 0) {
            ...
            // ONTEM
            $abc = $this->pdo->query("SELECT id, nome, cidade, estado, nascimento, email FROM `clientes` WHERE nascimento LIKE '%-".$ontem->format('m-d')."'");
            if($abc->rowCount() > 0) {
                while($reg = $abc->fetch(\PDO::FETCH_OBJ)) {
                    $x = new \DateTime($reg->nascimento);
                    $reg->nascimento_str = $x->format('d/m/Y');
            ...
            while($dias < 7) {
                ...
                $query = "SELECT id, nome, cidade, estado, nascimento, email FROM `clientes` WHERE nascimento LIKE '%-".$amanha->format('m-d')."' ";
            ...
            if($hoje->format('m-d') == '03-01' && $ontem->format('m-d') == '02-28') {
                ...
                $abc = $this->pdo->query("SELECT id, nome, cidade, estado, nascimento, email FROM `clientes` WHERE nascimento LIKE '%-02-29'");
                if($abc->rowCount() > 0) {
        ...
        foreach($listaRoteiros as $r) {
            ...
            foreach($venda->items as $items) {
                if($items->roteiroID === $r) {
                    ...
                    // verifica se está definido criança de colo na tarifa.
                    if(!isset($roteiro->tarifa[$x]->distr->colo)) {$roteiro->tarifa[$x]->distr->colo = 0;}

                    // Conta clientes.
                    $v->criancas += ( (int)$roteiro->tarifa[$x]->distr->criancas * (int)$items->qtd );
                    $v->adultos += ( (int)$roteiro->tarifa[$x]->distr->adultos * (int)$items->qtd );
                    $v->criancas_colo += ( (int)$roteiro->tarifa[$x]->distr->colo * (int)$items->qtd );
                    ...
                    $items->criancas_colo = ( (int)$roteiro->tarifa[$x]->distr->colo * (int)$items->qtd );
            ...
            $v->clientes_total = $v->criancas + $v->adultos + $v->criancas_colo;
            //print_r($v);

            // Verifica se há estoque suficiente para efetuar a compra. Criança de colo não inclusa na contagem.
            if($roteiro->estoque['total'] - $roteiro->estoque['vendidos'] >= $v->clientes_total - $v->criancas_colo) {
            
                // Lança venda no sistema.
                if($v->valor_total == 0) { // Venda cortesia: PAGA.
                    $dataVenda = 'NOW()';
                    $dataPagamento = 'NOW()';
                } else if($formaPagamento !== 'Reserva') { // Aguardando pagamento
                    $dataVenda = 'NOW()';
                    $dataPagamento = 'NULL';
                } else { // Reserva
                    $dataVenda = 'NULL';
                ...
                $abc = $this->pdo->prepare("INSERT INTO `vendas` (`id`, `cliente_id`, `roteiro_id`, `items`, `adultos`, `criancas`, `criancas_colo`, `clientes_total`, `lista_clientes`, ".
                "`desconto_total`, `valor_total`, `valor_devolvido`, `status`, `obs`, `forma_pagamento`, `parcelas`, `parcelas_pagas`, `total_pago`, `detalhes_pagamento`, `vencimento`, ".
                "`usuario_id`, `data_reserva`, `data_venda`, `data_pagamento`, `data_cancelado`, `data_estorno`) VALUES ".
                "(NULL, :cliente, :roteiro, :items, :adultos, :criancas, :crian_colo, :clientesTotal, '', :descontoTotal, :valorTotal, NULL, :status, :obs, :formaPagamento, :parcelas, :parcelasPagas, 0, :detPagam, ".
                ":venc, $vendedor, NOW(), $dataVenda, $dataPagamento, NULL, NULL)");

                /**
                 * Verifica o valor total, para definir se é uma reserva ou venda cortesia/brinde.
                 */
                if($v->valor_total == 0) { // VALOR CORTESIA/BRINDE: Se o valor total da venda/reserva for igual a 0, muda o estado automaticamente para PAGA.
                    ...
                    $abc->bindValue(':crian_colo', $v->criancas_colo, \PDO::PARAM_INT);
                    ...
                    $abc->bindValue(':status', 'Paga', \PDO::PARAM_STR);
                    $abc->bindValue(':formaPagamento', 'Outro', \PDO::PARAM_STR);
                    
                    $abc->bindValue(':parcelas', $parcelas, \PDO::PARAM_INT);
                    $abc->bindValue(':parcelasPagas', $parcelas, \PDO::PARAM_INT);
                    $abc->bindValue(':venc', $vencimento, \PDO::PARAM_STR);
                    $abc->bindValue(':detPagam', json_encode([
                        (object)['valor' => $v->valor_total, 'parcela' => 1, 'data' => date('Y-m-d')]
                    ]), \PDO::PARAM_STR);

                } else { // Vendas/reservas normais.
                    $abc->bindValue(':cliente', $cliente, \PDO::PARAM_INT);
                    $abc->bindValue(':roteiro', $v->roteiroID, \PDO::PARAM_INT);
                    $abc->bindValue(':items', json_encode($v->items), \PDO::PARAM_STR);
                    $abc->bindValue(':adultos', $v->adultos, \PDO::PARAM_INT);
                    ...
                    $abc->bindValue(':parcelas', $parcelas, \PDO::PARAM_INT);
                    $abc->bindValue(':parcelasPagas', 0, \PDO::PARAM_INT);
                    $abc->bindValue(':venc', $vencimento, \PDO::PARAM_STR);
                    $abc->bindValue(':detPagam', '', \PDO::PARAM_STR);
                }
